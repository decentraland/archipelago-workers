...
- **Clusters are uncapped, and nothing downstream splits them.** Nothing bounds co-located crowd size server-side any more, so the client's GPU becomes the binding constraint (unity-explorer's crowd-ghost work). comms-gatekeeper shipped without room sharding — one cluster maps to one LiveKit room, `island-{clusterId}` — so a percolated cluster becomes one oversized room; tracked as an open question in `Pulse/docs/clustering-on-aoi.md`.
